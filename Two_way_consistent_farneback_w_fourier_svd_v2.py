...
def svd_filter_images(image_folder, remove_modes, do_plots=True):
    print("\n Loading TIFF images...")
...
def run_optical_flow(images, image_files, output_folder_global):
...
if __name__ == "__main__":
    ...
    # Create output dirs
    output_folder = os.path.join(output_folder_global, os.path.basename(image_folder) + f"_run_{runID}")
    ...
    filtered_volume, image_files = svd_filter_images(image_folder, remove_modes=KillIDs, do_plots=False)
    images = [filtered_volume[i] for i in range(filtered_volume.shape[0])]

    # Step 2: Optical flow
    run_optical_flow(images, image_files, output_folder_global)
